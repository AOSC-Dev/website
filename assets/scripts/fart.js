...
const baseUrl = 'https://repo.aosc.io/misc/farts/'
...
Fart.prototype.play = function(sound, callback) {
	...
	if (!this.fart_old_player) {
		...
		this.fart_player.setAttribute('src', baseUrl + fart + ext);
		...
		this.fart_player.URL = baseUrl + fart + '.mp3';
	}
};
...
Fart.prototype.preload = function() {
	...
	if (!this.fart_old_player && !Fart.preloaded) {
		for(const f in farts){
			...
			fart_player.setAttribute('src', baseUrl + farts[f] + ext);
